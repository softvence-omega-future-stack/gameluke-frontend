...
// Mock Data
const stats = [
    { label: "Total Scores Today", value: "04", icon: CheckCircle2, iconColor: "text-blue-500", iconBg: "bg-blue-500/10", borderColor: "border-blue-500/30" },
    { label: "Edited Scores", value: "01", icon: Edit2, iconColor: "text-yellow-500", iconBg: "bg-yellow-500/10", borderColor: "border-yellow-500/30" },
    { label: "Changes Today", value: "01", icon: History, iconColor: "text-purple-500", iconBg: "bg-purple-500/10", borderColor: "border-purple-500/30" },
];
...
const currentScores = [
    {
        ...
        stationDetails: [
            { name: "Station A", team: "Team 01", score: 150, players: ["John Doe", "John Doe"] },
            { name: "Station B", team: "Team 02", score: 130, players: ["Jane", "Jane"] },
            { name: "Station C", team: "Team 02", score: 145, players: ["Tom Latham", "Tom Latham"] },
        ]
    },
    { id: 2, name: "Team Phoenix", room: "Room 1", stations: "Station A,B,C", games: "Basketball", time: "13m ago", score: 435, isEdited: true },
    { id: 3, name: "Alpha", room: "Room 1", stations: "Station A,B,C", games: "Basketball", time: "13m ago", score: 420, isEdited: false },
    { id: 4, name: "Omega", room: "Room 1", stations: "Station A,B,C", games: "Basketball", time: "13m ago", score: 430, isEdited: false },
];
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6">
            ...
            <div className="relative w-full max-w-lg bg-bg-card border border-zinc-800 rounded-2xl p-6 sm:p-8 shadow-2xl animate-in zoom-in-95 duration-200">
                <h3 className="text-xl sm:text-2xl font-black text-white uppercase tracking-tight italic mb-2">Confirm Score Changes</h3>
                <p className="text-zinc-400 text-sm mb-6">Please provide a reason for this change for the audit log.</p>

                <div className="space-y-4">
                    <div>
                        <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest block mb-2">Reason for Change</label>
                        <textarea
                            autoFocus
                            value={reason}
                            onChange={(e) => setReason(e.target.value)}
                            placeholder="e.g., Sensor calibration error, manual override required..."
                            className="w-full bg-bg-deep border border-zinc-700 rounded-xl py-3 px-4 text-white text-sm focus:outline-none focus:border-brand-secondary transition-all min-h-[100px] resize-none"
                        />
                    </div>
            ...
            <div className="relative w-full max-w-4xl bg-bg-deep border border-zinc-800 rounded-3xl overflow-hidden flex flex-col max-h-[90vh] shadow-2xl animate-in zoom-in-95 duration-200">
                ...
                <div className="flex-1 overflow-y-auto p-4 sm:p-8 space-y-6 scrollbar-hide">
                    ...
                    <div className="bg-[#2D0B2E] border-border p-6 relative overflow-hidden group">
                        <div className="flex flex-col sm:flex-row justify-between items-center gap-4 relative z-10">
                            <div className="flex items-center gap-4 text-center sm:text-left">
                                ...
                                <div>
                                    <p className="text-zinc-400 text-[10px] font-bold uppercase tracking-widest">Time Remaining</p>
                                    <h4 className="text-white text-2xl font-black">{score.timeRemaining || "00:00"}</h4>
                                </div>
                            </div>

                            <h3 className="text-brand-secondary text-2xl sm:text-3xl font-black uppercase tracking-tighter text-center italic">
                                {score.gameName || "Basketball Game"}
                            </h3>

                            <div className="text-center sm:text-right">
                                <p className="text-zinc-400 text-[10px] font-bold uppercase tracking-widest">Total Score</p>
                                <h4 className="text-white text-3xl font-black">
                                    {editableDetails.reduce((acc, curr) => acc + (Number(curr.score) || 0), 0)}
                                </h4>
                            </div>
                        </div>
                    ...
                    <div className="bg-bg-card border border-zinc-800 rounded-2xl p-5">
                        <p className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest mb-1">Playing as</p>
                            ...
                            <div key={idx} className="bg-bg-card border-border p-4 sm:p-6 space-y-4">
                                <div className="flex justify-between items-start">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <h4 className="text-brand-secondary text-xl font-black uppercase italic tracking-tight">{station.name}</h4>
                                            <p className="text-zinc-500 text-xs font-bold uppercase">{station.team}</p>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-6">
                                        <div className="text-right">
                                            <p className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest mb-1">Score</p>
                                            <input
                                                type="number"
                                                value={station.score}
                                    ...
                                    {station.players.map((player, pIdx) => (
                                        <div key={pIdx} className="bg-bg-deep border-border p-3 flex items-center gap-3 group/player">
                                            <div className={cn(
                                                ...
                                                {pIdx % 2 === 0 ? <User className="w-4 h-4 text-brand-accent" /> : <Trophy className="w-4 h-4 text-brand-info" />}
                                            </div>
                                            <span className="text-zinc-300 text-sm font-bold uppercase tracking-wide">{player}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                    ...
                    <div className="pt-4">
                        <button
                            ...
                            className="w-full bg-brand-secondary text-black py-4 rounded-2xl font-black text-lg uppercase tracking-widest hover:bg-brand-secondary/90 transition-all shadow-lg shadow-brand-secondary/20 active:scale-[0.98] cursor-pointer"
                        >
                            Save Changes
                        </button>
                    </div>
...
export default function ScoreManagementPage() {
    ...
    const [selectedScore, setSelectedScore] = useState<typeof currentScores[0] | null>(null);
                            ...
                            <div key={index} className="bg-bg-deep border-border rounded-xl p-4 sm:p-5 relative group transition-colors">
                                <div className="flex flex-col sm:flex-row justify-between gap-4">
                                    <div className="space-y-2">
                                        ...
                                        <div className="flex items-center gap-4">
                                            <span className="text-yellow-500/80 line-through text-lg sm:text-xl font-bold">{item.oldScore}</span>
                                            <ArrowRight className="w-4 h-4 text-zinc-600" />
                                            <span className="text-green-500 text-xl sm:text-2xl font-bold">{item.newScore}</span>
                                        </div>
                                        <div className="flex items-center gap-2 text-brand-success">
                                            <AlertCircle className="w-3.5 h-3.5" />
                                            <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider">{item.reason}</span>
                                        </div>
                                    </div>
                                    <div className="text-right shrink-0">
                                        <p className="text-zinc-500 text-[10px] sm:text-xs font-bold uppercase tracking-wider">by {item.editedBy}</p>
                                        <p className="text-brand-success text-[10px] sm:text-xs font-medium mt-1 uppercase tracking-wider">{item.time}</p>
                                    </div>
                                </div>
                            </div>
                        ))}
                        ...
                        >
                            <div className="space-y-2 w-full sm:w-auto">
                                <div className="flex items-center gap-3">
                                    <h3 className="text-lg sm:text-xl font-bold text-white">{row.name}</h3>
                                    {row.isEdited && (
                                ...
                                <div className="grid grid-cols-2 lg:flex items-center gap-x-4 gap-y-1 text-zinc-500 text-[10px] sm:text-xs font-bold uppercase tracking-wider">
                                    <p>Room: <span className="text-zinc-300 ml-1">{row.room}</span></p>
                                    <p>Station: <span className="text-zinc-300 ml-1">{row.stations}</span></p>
                                    <p>Games: <span className="text-zinc-300 ml-1">{row.games}</span></p>
                                    <p className="col-span-2 sm:col-span-1">Time: <span className="text-zinc-300 ml-1">{row.time}</span></p>
                                </div>
                            </div>
                            <div className="flex items-center justify-between sm:justify-end w-full sm:w-auto gap-6 sm:gap-10 border-t sm:border-t-0 border-zinc-800/50 pt-4 sm:pt-0">
                                <div className="text-left sm:text-right">
                                    ...
                                    <span className="text-brand-secondary text-xl sm:text-2xl font-extrabold">{row.score}</span>
                                </div>
                ...
                <ScoreDetailsModal
                    ...
                    onSave={(newDetails, reason) => {
                        ...
                        if (selectedScore) {
                            ...
                            const newTotal = cleanedDetails.reduce((acc, curr) => acc + curr.score, 0);

                            selectedScore.score = newTotal;
                            (selectedScore as any).stationDetails = cleanedDetails;
                            selectedScore.isEdited = true;

                            // Proactively add to history (mock)
                            console.log(`Score saved for ${selectedScore.name}. Reason: ${reason}`);
                        }
